...
import { clearMessage } from '../redux/actions/systemAction';
...
export function SystemMessage() {
    ...
    if (systemMessage.type === 'success') {
        return (
            <Modal close={() => dispatch(clearMessage())} textColor={'text-green-500'}>{systemMessage.message}</Modal>
        )
    }
    if (systemMessage.type === 'error') {
        return (<Modal close={() => dispatch(clearMessage())}>{systemMessage.message}</Modal>)
    }
    return null
}
